 ...
 * 4. Issue X.509-backed W3C credential from UNTRUSTED issuer → stored
...
import { GOV_CREDENTIAL_IDS } from '../gov-services-config.js';
...
export async function flowGovTrust(ctx: CommandContext): Promise<void> {
  ...
  const untrustedProfilePath = `${organization}.${untrustedTenantId}.untrusted-issuer.employee`;
  ...
  try {
    ...
    console.log('\n--- Step 4: Issue Employee Status from UNTRUSTED issuer (stored for verifier negative case) ---');
