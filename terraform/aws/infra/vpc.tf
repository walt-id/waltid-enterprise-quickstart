...
resource "aws_eip" "nat" {
  ...
  tags = merge(local.common_tags, { Name = "${var.cluster_name}-nat-eip-${count.index + 1}" })
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = merge(local.common_tags, { Name = "${var.cluster_name}-nat-${count.index + 1}" })
...
resource "aws_route_table" "private" {
  ...
  tags = merge(local.common_tags, { Name = "${var.cluster_name}-private-rt-${count.index + 1}" })
}
